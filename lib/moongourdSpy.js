...
module.exports = function MoongourdParser(mod) {
	...
	const getLink = (serverId, playerName, continent, boss, mgVer) => {
		let region = "EU";
		switch (serverId) {
			case (43):
			case (42): region = "NA"; break;
		}
		return `https://moongourd.com/api/mg/search.php?region=${region}&zone=${continent}&boss=${boss}&ver=${mgVer}&name=${playerName}&page=1`
	};

	const requestData = async (link) => {
		//const requestPayload = await fetch(link);
		//tmpfix CERT_HAS_EXPIRED?
		const requestPayload = await fetch(link, { agent });
	...
	const analyzeChar = async (charName, continentId, bossId, dungeonName, mgVer, expectedDps) => {
		const link = getLink(mod.serverId, charName, continentId, bossId, mgVer);
	...
	const startWrappedCheck = async (name) => {
		if (mod.settings.keepContinentsSequence)
			for(const x of mod.settings.continentsToCheck) await analyzeChar(name, x.continent, x.boss, x.name, x.mgVer, x.expectedDpsAvg);
		else 
			for(const x of mod.settings.continentsToCheck) analyzeChar(name, x.continent, x.boss, x.name, x.mgVer, x.expectedDpsAvg);
	};

	mod.hook('S_USER_PAPERDOLL_INFO', 16, event => {
